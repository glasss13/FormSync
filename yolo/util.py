...
def predict_transform(prediction, inp_dim, anchors, num_classes, device=None):
    """
    Transforms the raw output from a YOLO layer into bounding box predictions.

    Args:
        prediction (torch.Tensor): The raw output tensor from a YOLO layer.
        inp_dim (int): The input dimension of the network.
        anchors (list): A list of anchor box dimensions.
        ...
        device (torch.device or str, optional): The device to perform operations on.
                                                 Defaults to None, which infers from prediction's device.

    Returns:
        torch.Tensor: A tensor containing processed bounding box predictions.
    """
    batch_size = prediction.size(0)
    stride = inp_dim // prediction.size(2)
    grid_size = prediction.size(2) # Assuming square grid
    ...
    grid_len = torch.arange(grid_size, device=device) # Create on device
    a, b = torch.meshgrid(grid_len, grid_len, indexing='ij') # Use torch.meshgrid and specify indexing

    x_offset = a.reshape(-1, 1) # Use reshape instead of view for clarity
    y_offset = b.reshape(-1, 1)

    x_y_offset = torch.cat((x_offset, y_offset), 1).repeat(1, num_anchors).view(-1, 2).unsqueeze(0)

    # Ensure x_y_offset is on the correct device (it should be if grid_len was)
    x_y_offset = x_y_offset.to(device)

    prediction[:, :, :2] += x_y_offset

    # log space transform height and the width
    anchors = torch.FloatTensor(anchors).to(device) # Create on device

    anchors = anchors.repeat(grid_size * grid_size, 1).unsqueeze(0)
    prediction[:, :, 2:4] = torch.exp(prediction[:, :, 2:4]) * anchors

    # Softmax (or Sigmoid for multi-label) the class scores
    # YOLOv3 typically uses Sigmoid for class scores
    prediction[:, :, 5: 5 + num_classes] = torch.sigmoid((prediction[:, :, 5: 5 + num_classes]))

    # Scale bounding boxes to the original input dimension
    prediction[:, :, :4] *= stride
